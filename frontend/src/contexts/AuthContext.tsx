...
export function AuthProvider({ children }: AuthProviderProps): React.JSX.Element {
  ...
  // Set error helper
  const setErrorHelper = (value: string | null) => {
    setError(value)
  }
  ...
  const handleAuthExpired = () => {
    ...
    setUser(null)
    setErrorHelper('Session expired. Please login again.')
    router.push('/login')
  }
  ...
  const login = async (credentials: LoginRequest): Promise<boolean> => {
    try {
      setIsLoading(true)
      setErrorHelper(null) // Clear previous error at start of new attempt
      ...
      if (result.success && result.data) {
        ...
        if (result.error instanceof AuthInvalidError) {
          setErrorHelper(result.error.message)
        } else if (result.error instanceof NetworkError) {
          setErrorHelper('Network error. Please check your connection.')
        } else {
          setErrorHelper(result.error.message || 'Login failed. Please try again.')
        }
        return false  // Login failed
      }

      // Should not reach here, but handle just in case
      setErrorHelper('Unexpected login error')
      return false
    } catch (error) {
      setErrorHelper('Login failed. Please try again.')
      return false
    } finally {
      setIsLoading(false)
    }
  ...
  const logout = async (): Promise<void> => {
    try {
      ...
      // Always clear user state and tokens even if API call fails
      TokenStorage.clearTokens()
      setUser(null)
      setIsLoading(false)
    ...
    try {
      if (checkAuthStatus()) {
        ...
        if (result.success && result.data) {
          ...
          setErrorHelper('Failed to refresh user information')
        }
      }
    } catch (error) {
      setErrorHelper('Failed to refresh user information')
