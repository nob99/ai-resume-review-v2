...
if [ ! -f "$TERRAFORM_DIR/main.tf" ]; then
    log_error "Not in terraform/environments/production directory"
...
import_resource \
    "Cloud SQL Instance" \
    "module.production_environment.google_sql_database_instance.postgres" \
    "ai-resume-review-v2-db-prod"

# Import Database
import_resource \
    "Database" \
    "module.production_environment.google_sql_database.database" \
    "projects/$PROJECT_ID/instances/ai-resume-review-v2-db-prod/databases/ai_resume_review_prod"
...
import_resource \
    ...
    "module.production_environment.google_service_account.backend" \
    "projects/$PROJECT_ID/serviceAccounts/arr-v2-backend-prod@ytgrs-464303.iam.gserviceaccount.com"

# Import Frontend Service Account
import_resource \
    "Frontend Service Account" \
    "module.production_environment.google_service_account.frontend" \
    "projects/$PROJECT_ID/serviceAccounts/arr-v2-frontend-prod@ytgrs-464303.iam.gserviceaccount.com"
...
import_resource \
    ...
    "module.production_environment.google_artifact_registry_repository.docker" \
...
import_resource \
    ...
    "module.production_environment.google_project_iam_member.backend_sql_client" \
    "$PROJECT_ID roles/cloudsql.client serviceAccount:arr-v2-backend-prod@ytgrs-464303.iam.gserviceaccount.com" \
...
import_resource \
    ...
    "module.production_environment.google_project_iam_member.backend_secret_accessor" \
    "$PROJECT_ID roles/secretmanager.secretAccessor serviceAccount:arr-v2-backend-prod@ytgrs-464303.iam.gserviceaccount.com" \
    || log_warning "IAM binding might not exist - will be created on apply"

# Backend → Log Writer
import_resource \
    "Backend Log Writer Role" \
    "module.production_environment.google_project_iam_member.backend_log_writer" \
    "$PROJECT_ID roles/logging.logWriter serviceAccount:arr-v2-backend-prod@ytgrs-464303.iam.gserviceaccount.com" \
    || log_warning "IAM binding might not exist - will be created on apply"

# Frontend → Log Writer
import_resource \
    "Frontend Log Writer Role" \
    "module.production_environment.google_project_iam_member.frontend_log_writer" \
    "$PROJECT_ID roles/logging.logWriter serviceAccount:arr-v2-frontend-prod@ytgrs-464303.iam.gserviceaccount.com" \
    || log_warning "IAM binding might not exist - will be created on apply"
